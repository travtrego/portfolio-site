...
export const dataProjects: Project[] = [
  ...
  },
  {
    ...
    description:
      "A screening model for greenwashing risk in corporate climate claims, built from 2025 sustainability and environmental performance reports from three major airlines. Each claim is scored against five credibility indicators — a stated number, target year, baseline year, emissions scope, and progress evidence — with only the indicators actually applicable to that type of claim counted, so a claim isn't penalized for missing a scope reference it was never expected to have. Claims that back up fewer of their applicable indicators score higher on a 0–5 Greenwashing Risk Score. Built, documented, and presented using Python/pandas pipelines (verified to reproduce the original scored output row-for-row before trusting the refactor) and Next.js/Recharts.",
